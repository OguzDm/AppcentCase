...
final class SpiritViewController: UIViewController,FilterViewControllerDelegate {
    
    func didUpdateFilter(filter: String) {
        queryParams["camera"] = filter
        currentPage = 1
        self.PHOTOS.removeAll()
        Service.shared.fetchRequest(rover: .Spirit,queryParams: queryParams,page: currentPage,sol: sol) { rover in
    ...
    override func viewDidLoad() {
        ...
        Service.shared.fetchRequest(rover: .Spirit,queryParams: queryParams, page: 1,sol: sol) { rover in
            self.PHOTOS.append(contentsOf: rover.photos)
            DispatchQueue.main.async {
                self.collectionView.reloadData()
            }
